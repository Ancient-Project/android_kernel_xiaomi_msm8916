...
#ifdef XIAOMI_PRODUCT
	ret = ist30xx_read_reg(data->client,
			IST30XX_DA_ADDR(eHCOM_GET_LOCKDOWN_1), &data->fw.cur.lockdown[0]);
	if (unlikely(ret))
		goto err_get_ver;

	ret = ist30xx_read_reg(data->client,
			IST30XX_DA_ADDR(eHCOM_GET_LOCKDOWN_2), &data->fw.cur.lockdown[1]);
	if (unlikely(ret))
		goto err_get_ver;

	ret = ist30xx_read_reg(data->client,
			IST30XX_DA_ADDR(eHCOM_GET_CONFIG_1), &data->fw.cur.config[0]);
	if (unlikely(ret))
		goto err_get_ver;

	ret = ist30xx_read_reg(data->client,
			IST30XX_DA_ADDR(eHCOM_GET_CONFIG_2), &data->fw.cur.config[1]);
	if (unlikely(ret))
		goto err_get_ver;
#endif
...
/*#ifdef CONFIG_WAKE_GESTURES
		...
		if ((finger_press == false) && (key_press == false)) {
			...
			if (finger_press == true) {
				tsp_info("finger press id: %d ( %d, %d)\n", id, fingers[idx].bit_field.x, fingers[idx].bit_field.y);
				input_mt_slot(data->input_dev, id);
				input_mt_report_slot_state(data->input_dev, MT_TOOL_FINGER, true);
				input_report_abs(data->input_dev, ABS_MT_POSITION_X, fingers[idx].bit_field.x);
				input_report_abs(data->input_dev, ABS_MT_POSITION_Y, fingers[idx].bit_field.y);
			...
			if ( (key_press == true) && (data->disable_keys == false) ) {
				...
				input_mt_slot(data->input_dev, 0);
				input_mt_report_slot_state(data->input_dev, MT_TOOL_FINGER, true);
				input_report_abs(data->input_dev, ABS_MT_POSITION_X, fhd_key_dim_x[id + 1]);
				input_report_abs(data->input_dev, ABS_MT_POSITION_Y, FHD_KEY_Y);
				input_report_abs(data->input_dev, ABS_MT_TOUCH_MAJOR, IST30XX_MAX_W);
			}
		}
	}
...
static int ist30xx_resume(struct device *dev)
{
	...
	ist30xx_internal_resume(data);
		...
		if (!batt_psy)
			...
			tsp_info("is_charger_plug %d, prev %d", is_charger_plug, pre_charger_status);
			if (is_charger_plug) {
...
{
	struct ist30xx_data *data = container_of(work, struct ist30xx_data, fb_notify_work);
...
void timer_handler(unsigned long timer_data)
{
	...
	if (data->irq_working || !data->initialized)
		goto restart_timer;
...
static int ist30xx_ts_pinctrl_init(struct ist30xx_data *ist30xx_data)
{
	...
	ist30xx_data->ts_pinctrl = devm_pinctrl_get(&(ist30xx_data->client->dev));
	...
	ist30xx_data->gpio_state_active
		= pinctrl_lookup_state(ist30xx_data->ts_pinctrl,
							   "pmx_ts_active");
...
static int ist30xx_ts_pinctrl_select(struct ist30xx_data *ist30xx_data, bool on)
{
	...
	if (!IS_ERR_OR_NULL(pins_state)) {
		ret = pinctrl_select_state(ist30xx_data->ts_pinctrl, pins_state);
		if (ret) {
			dev_err(&ist30xx_data->client->dev,
					"can not set %s pins\n",
...
*/
	/* PID info read */
		...
		while (retry-- > 0) {
			...
			if (ret == 0) {
				if ((info_data[1] & XIAOMI_INFO_MASK) == XIAOMI_INFO_VALUE) {
					data->product_id = XIAOMI_GET_PID(info_data[1]);
					data->lockdown_upper = info_data[0];
					break;
				} else if ((info_data[0] & XIAOMI_INFO_MASK) == XIAOMI_INFO_VALUE) {
	...
	retry = IST30XX_MAX_RETRY_CNT;
	while (retry-- > 0) {
		ret = ist30xx_read_reg(data->client, IST30XX_REG_CHIPID, &data->chip_id);
		data->chip_id &= 0xFFFF;
		if (unlikely(ret == 0)) {
			if ((data->chip_id == IST30XX_CHIP_ID) || (data->chip_id == IST30XXC_DEFAULT_CHIP_ID) || (data->chip_id == IST3048C_DEFAULT_CHIP_ID)) {
				break;
			}
